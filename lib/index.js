...
var debug = require('debug')('nodemail-mail');
...
exports.sendActiveMail = function (who, token, name) {
    ...
    var to = who;
    var subject = config.name + '回信';
    var html = '<p>您好：' + name + '</p>' +
        '<p>我们收到您对' + config.name + '的思念，特发此邮件已表慰问，' + config.name + '也十分想你</p>' +
            //'<a href  = "' + SITE_ROOT_URL + '/active_account?key=' + token + '&name=' + name + '">激活链接</a>' +
        '<p>若您对' + config.name + '并没有思念，说明有人滥用了您的电子邮箱，请删除此邮件，我们对给您造成的打扰感到抱歉。</p>' +
        '<p>' + config.name + ' 谨上。</p>';
    //var html = '<p>您好：' + name + '</p>' +
